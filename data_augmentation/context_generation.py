...
parser.add_argument("--filterOut", type=str, help="Specify the filter out")
...
original_index_name = index_name.split("_short")[0].split("_long")[0].split("Filtered")[0]
...
qac_index_path = os.path.join(os.environ['HOME'], 'code', 'data_augmentation', 'qac_dataset', index_name + ".json")
print("QAC Index Path:", qac_index_path)
with open(qac_index_path, 'r') as file:
    data = json.load(file)

questions_qac = data["questions"]
text_corpus_qac = data["contexts"]
answers_qac = data["answers"]
...
for i, question in tqdm(enumerate(questions_qac), total=len(questions_qac), desc="Processing questions", file=sys.stdout):
    outer_index, inner_index = find_string_index(questions, question)
    question_embedding = questions_embeddings[outer_index][inner_index]
    new_contexts = []
    # Check if text_corpus_qac[i] is string or list
    if type(text_corpus_qac[i]) == str:
        new_contexts.append(text_corpus_qac[i])
    elif type(text_corpus_qac[i]) == list:
        new_contexts.extend(text_corpus_qac[i])
    new_contexts.extend(retriever.get_top_k_documents(question_embedding, k=10, scores=False))
    ...
    if filter_out:
        ...
        if text_corpus_qac[i] in tousand_most_similar_documents:
            retrieved_contexts.append(new_contexts)
        else: 
...
if filter_out:
    ...
    print("")
    print(f"Number of removed questions: {len(index_to_drop)}")
    print("")
    questions_qac = [question for i, question in enumerate(questions_qac) if i not in index_to_drop]
    answers_qac = [answer for i, answer in enumerate(answers_qac) if i not in index_to_drop]
    retrieved_contexts = [context for i, context in enumerate(retrieved_contexts) if i not in index_to_drop]

### Save the qac_extended json
save_path = os.path.join(os.environ['HOME'], 'code', 'data_augmentation', 'qac_extended_dataset', index_name + ".json")

print("Saving to:", save_path)

with open(save_path, 'w') as file:
    json.dump({"questions": questions_qac, "contexts": retrieved_contexts, "answers": answers_qac}, file, indent=2)