...
class LargeDPROpenAIRetriever(BaseRetriever):
    ...
    def get_top_k_documents(self, query, k=1000, scores=False):
        """
...
# TODO: Debbuging comment
# print(f"Number Text Corpus Entries: {len(text_corpus)} | Number Questions: {total_questions}")

# # random sample 1000 question and text_corpus pairs
# import random
# random.seed(42)
# # The same shuffle needs to be applied on both lists
# text_corpus, questions = zip(*random.sample(list(zip(text_corpus, questions)), 1000))
...
def calculate_hit_ratio_and_mrr_parallel(args):
    ...
    for i, question_set in tqdm(enumerate(questions), total=len(questions), desc=f"Processing questions for k: {k} and #questions: {len(questions)}", file=sys.stdout):
        for question in question_set:
            ...
            try:
                if ensemble:
                    top_k_documents, document_scores = retriever.get_top_k_documents(question, k=k, scores=True)
...
if retriever == "BM25":
    ...
    k_values = k
    unpacked_list = calculate_hit_ratio_and_mrr_parallel_wrapper(bm25_retriever, text_corpus, questions, k_values,ensemble=True, multithreading=False)
